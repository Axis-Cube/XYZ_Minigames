...
export function getRawPrice(id) {
    ...
    if (item.price == -1 || item.price == -2 || item.price == -3)
        return item.price;
                    ...
                    if (!gl.canceled) {
                        if (gl.selection == 0) {
                            const form3 = new ModalFormData()
                                ...
                                .show(player).then(ggwp => { if (!ggwp.canceled) {
                                formItemsTransfer(player);
                            } });
                        }
                        else {
        ...
        if (!gg.canceled) {
            ...
            if (gg.formValues[0].split('.').length == 1) {
                ...
                if (key == generateTransferCode(id, player.name)) {
                    ...
                    form2.show(player).then(async (gl) => {
                        if (!gl.canceled) {
...
export function formItemInfo(itemId, player) {
    ...
    if (item.type == 'setpack') {
        ...
        try {
            ...
            if (item.price > 0) {
                if (!isPurchased)
                    form.button(`%skins.buy.buyButton${getPrice(itemId, true, name)}`);
                text.rawtext.push({ translate: `axiscube.store.product_info.price`, with: [`${SCOLOR}${item.price}${SYM}`] });
            }
            else if (item.price == 0) {
                if (!isPurchased)
                    form.button(`%store.redeem`);
                text.rawtext.push({ translate: `axiscube.store.product_info.price`, with: { rawtext: [{ translate: 'store.free' }] } });
            }
            else if (item.price == -2) {
                if (!isPurchased)
                    form.button('%axiscube.store.price.unvaliable');
                text.rawtext.push({ translate: `axiscube.store.product_info.price`, with: { rawtext: [{ translate: 'axiscube.store.price.unvaliable' }] } });
            }
        ...
        if (item.price >= 0 && item.linkeds) {
            if ((item.linkeds).length == 1) {
                if (!isPurchased)
                    placeProductButton(item.linkeds[0], form, true, name);
                ...
                for (let i of item.linkeds) {
                    if (!isPurchased)
                        placeProductButton(i, form, true, name);
    ...
    form.show(player).then(async (gg) => {
        if (!gg.canceled) {
            if (gg.selection == 0) {
                if (item.linked && (!isPurchased && (item.type != 'setpack'))) {
                    formItemInfo(item.linked, player);
                    return;
                }
...
export function formPurchasedItems(player, comment = '', sort = 'date_newest') {
    ...
    form.show(player).then(gg => {
        if (gg.canceled) {
            ...
            if (sort == 'type') {
                formPurchasedItems(player, '', 'date_newest');
            }
            else if (sort == 'date_newest') {
                formPurchasedItems(player, '', 'date_oldest');
            }
            else if (sort == 'date_oldest') {
                formPurchasedItems(player, '', 'type');
            }
        }
        else if (gg.selection > 1 && items.length > 0) {
...
export function getPurchasedItems(name) {
    ...
    let final = [];
    for (let i of items) {
        if (generateTransferCode(i, name) == getSsData(name)[i.toString(36)]) {
            final.push(i);
        }
    }
    return final;
