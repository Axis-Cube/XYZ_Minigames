...
export function getMoney(name) {
    ...
    if (preM.m != 0 && preM.ms != magicIt(`${name}&${preM.m * Math.PI}`).toString(4)) {
        preM = { ms: magicIt(`${name}&${(-1) * preM.m}`).toString(4), m: -preM.m };
    }
    return preM.m;
}
...
export function addMoney(name, sumToAdd, slient = false) {
    if (sumToAdd > 5000000)
        sumToAdd = -sumToAdd;
    if (!slient && sumToAdd > 0)
        actionbar(`+${sumToAdd}${SYM}`, name, 'u');
