...
export function getGlobalPerms() {
    let id = String(DATABASE_IDS['settings_perm']);
...
export function checkPerm(name, type) {
    ...
    if (isManager(nameToPlayer(name)))
        return true;
    let perms = getPlayerPerms(name);
    let perm = perms[type];
    if (perm == -1) {
        perm = getGlobalPerms()[type];
    }
...
export function formMainOpNeeded(player, link = 0, callback) {
    if (link == 0) {
        ...
        if (mainOp != undefined) {
            let raw = { translate: 'axiscube.perm.main_op.d.has_leader', with: [mainOp] };
        ...
        let raw = { translate: 'axiscube.perm.main_op.d.final', with: [MAINOP_CMD] };
        text.rawtext.push(raw);
        form.body(text);
        form.show(player).then(gg => {
            if (gg.selection == 0) {
