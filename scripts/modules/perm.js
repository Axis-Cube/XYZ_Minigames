...
export function getGlobalPerms() {
    return dbGetRecord(DATABASE_IDS['settings_perm'],DB_DEFAULT)
}

export async function editGlobalPerms(obj) {
    await dbSetRecord(DATABASE_IDS['settings_perm'],obj)
}
...
export function formLocalPerms(player,name) {
    ...
    form.show(player).then( async gg => { if (!gg.canceled) {
        for (let i in setable) {
            newperms[setable[i]] = gg.formValues[i] -1
        }
...
export function formMainOpNeeded(player,link=0,callback) {
    if (link == 0) {
        ...
        if (mainOp != undefined) {
            text.rawtext.push({translate:'axiscube.perm.main_op.d.has_leader',with:[mainOp]})
        } else {
            text.rawtext.push({translate:'axiscube.perm.main_op.d.no_leader'})
        }
        text.rawtext.push({text:'\n\n'})
        text.rawtext.push({translate:'axiscube.perm.main_op.d.final',with:[MAINOP_CMD]})
        form.body(text)
        form.show(player).then( gg => {
            if (gg.selection == 0) {
                if (callback != undefined) {
