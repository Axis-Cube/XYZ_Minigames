...
async function placeBlock(player) {
    ...
    if (hasTag(player, 'hns.temp0')) {
        await runCMD('tag @s remove hns.temp0', player);
        await runCMD(`titleraw @s actionbar {"rawtext":[{"text":"\ue158 §c"},{"translate":"axiscube.hns.placed.error"}]}`, player);
        return;
    }
    ;
    if (hasTag(player, 'hns.hider.hidden')) {
        return;
    }
    ;
...
export async function hnsKiller(killer, prey) {
    if (hasTag(prey, 'hns.hider'))
        tellraw(`{"rawtext":[{"translate":"axiscube.hns.death","with":{"rawtext":[{"text":"${prey.name}"},{"text":"${getPlayerColor(killer.name)}${killer.name}"},{"translate":"${HNS_BLOCKS[getScore(prey, 'hns.block')].name}"}]}}]}`, '@a');
}
