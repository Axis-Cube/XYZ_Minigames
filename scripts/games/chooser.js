...
export async function formGameChooser(player, type = 1, disableCheck = false) {
    ...
    if (type == 2 && [...world.getPlayers()].length < 2 && !disableCheck) {
        ...
        await formWarn.show(player).then(gg => { if (gg.selection == 0) {
            openJSON('gamesel', player);
        }
        else if (gg.selection == 1) {
            formGameChooser(player, type, true);
        } });
    }
    if (!disableCheck && type == 2 && [...world.getPlayers()].length < 2)
        return;
        ...
        formOfThisGame.show(player).then(async (gg) => {
            if (gg.selection == 0) {
                ...
                if (thisGame.ui_type == 'default') {
                    ...
                    if (!isCanceled && thisGame.cmd)
                        await runCMDs(thisGame.cmd);
                    if (!isCanceled)
                        await startGame(thisGame.for_start, player);
                }
            }
...
export async function formBeginGameConfirm(player, id = getGame()) {
    ...
    form.show(player).then(async (gg) => {
        if (gg.selection === 0 || gg.selection === undefined || !canStart) {
            return;
        }
...
export async function formCancelGameConfirm(player, id = getGame()) {
    const form = new MessageFormData()
        ...
        .button2('%gui.confirm');
    form.show(player).then(gg => {
        if (gg.selection === 0 || gg.selection === undefined)
            return;
        if (!checkPerm(player.name, 'stop')) {
            rawtext('axiscube.perm.denied.stop', player.name, 'translate', 'c');
            return;
        }
