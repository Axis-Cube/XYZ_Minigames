...
export async function pvpSetkit(player = '@a', t = getPVPselectedSet()) {
    ...
    if (countArrow != 0) {
        await runCMD(`replaceitem entity @s slot.hotbar ${slotBow} ${itemBow} 1`, player);
    }
    if (itemBow != 'air') {
        await runCMD(`replaceitem entity @s slot.inventory 0 arrow ${countArrow}`, player);
    }
    ...
    try {
        ...
        form.show(player).then(gg => {
            if (gg.selection != undefined) {
                if (gg.selection < SETS.length) {
                    pvpShowKitInfo(player, SETS[gg.selection]);
                }
                else if (gg.selection == SETS.length) {
                    pvpCreateKit(player);
                }
                else if (gg.selection == SETS.length + 1) {
                    pvpTemplatesMenu(player);
                }
            }
        });
    }
...
export async function pvpTick() {
    ...
    if (settingsEnchLoyalty)
        runCMD(`enchant @a[hasitem={location=slot.weapon.mainhand,item=trident}] loyalty ${settingsEnchLoyalty}`);
    if (settingsEnchInfinity)
        runCMD(`enchant @a[hasitem={location=slot.weapon.mainhand,item=bow}] infinity ${settingsEnchInfinity}`);
