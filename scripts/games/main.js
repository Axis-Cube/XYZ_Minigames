...
for (let i in GAMEDATA) {
    GAMETAGS = GAMETAGS.concat(GAMEDATA[i].tags);
}
;
...
export async function startGame(id, player, arn = getGameArena()) {
    const thisGame = GAMEDATA[id];
    if (!checkPerm(player.name, 'start')) {
        rawtext('axiscube.perm.denied.start', player.name, 'translate', 'c');
        return;
    }
    if (getGame() != 0) {
        rawtext('axiscube.games.startgame.already', player, 'translate');
        return;
    }
    ...
    await clearTags();
    try {
        await runCMDs(thisGame.pre_commands);
    }
    catch { }
    let commands = [
        'clear @a',
...
system.runInterval(async () => {
    ...
    for (let player of [...world.getPlayers()]) {
        ...
        if (isManager(player))
            countOps = countOps + 1;
        if (isManager(player) && !isTempManager(player))
            countTrueOps = countTrueOps + 1;
    }
    if (countOps == 0) {
 ...
 * @returns {void}
 */
export async function stopGame(id = getGame(), finishType = 'slient', finishVariables = undefined) {
    try {
        if (getGame() == 0 && finishType != 'slient')
            return;
...
export function onDeathInGame(player, id = getGame()) {
    if (id == 0) {
        powerTP(GAMEDATA[0].loc[0].spawn, player);
    }
    runCMDs(GAMEDATA[id].death_data.death_commands, player);
}
export async function killerCommands(killer, prey, projectile) {
    if (!GAMEDATA[getGame()].death_data.disable_notify) {
        killMessage(killer, prey, projectile);
    }
    if (typeof GAMEDATA[getGame()].death_data.killFunc == 'function')
        GAMEDATA[getGame()].death_data.killFunc(killer, prey);
}
