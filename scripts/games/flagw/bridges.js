...
export const GAMEDATA_FW_BRIDGES = {
    id: 9,
...
async function bridgeEquipment() {
    ...
    try {
        ...
        '[Bridges]' + console.warn(e), console.warn(e.stack);
    }
}
...
async function bridgeTick() {
    for (const player of [...world.getPlayers()]) {
        if (!player.hasTag('spec')) {
            ...
            if (isInBlueHole) {
                HoleHandlers('blue', player);
            }
            if (isInRedHole) {
                HoleHandlers('red', player);
            }
            if (blue_team == 0 && red_team != 0) {
                await WinHandle('red');
            }
            else if (red_team == 0 && blue_team != 0) {
                await WinHandle('blue');
            }
        }
    }
}
