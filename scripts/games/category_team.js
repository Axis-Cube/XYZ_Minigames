...
export function getPlayerTeam(player) {
    ...
    for (let i in playerTags) {
        if (playerTags[i].startsWith('team.'))
            return playerTags[i].split('.')[1];
    }
}
export function formTeamsel(player, teamsList = TEAMS, addSpectator = true, colorNameTag = true, iconType = 'heads') {
    ...
    for (let i in teamsList) {
        let isSel = '';
        if (!knowTeam && hasTag(player, `team.${teamsList[i]}`)) {
            isSel = '§l';
            knowTeam = true;
        }
    ...
    form.show(player).then(async (gg) => {
        if (!gg.canceled) {
            ...
            for (let i in playerTags) {
                if (playerTags[i].startsWith('team.')) {
                    await runCMD(`tag @s remove ${playerTags[i]}`, player);
                }
            }
            if (addSpectator) {
                ...
                if (gg.selection === 0) {
                    ...
                    if (colorNameTag)
                        player.nameTag = `${TEAM_COLORS[teamsList[Number(gg.selection) - 1]]}${player.name}§r`;
                ...
                if (colorNameTag)
                    player.nameTag = `${TEAM_COLORS[teamsList[Number(gg.selection)]]}${player.name}§r`;
