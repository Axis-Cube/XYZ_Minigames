...
const PVP_ICONS = {
    ...
    weapon: {
        ...
        misc: {
            0: ['\ue1c6','textures/ui/icons/items/air_misc'],
...
export async function pvpSetkit(player='@a',t=getPVPselectedSet()) {
    ...
    await runCMD(`replaceitem entity @s slot.hotbar 3 ${itemAxe} 1`,player) 
    // BOW && ARROWS
...
function pvpIconKit(obj) {
    ...
    let sword_icon = (obj.suit.weapon.sword <= Object.keys(PVP_ICONS.weapon.sword).length-1) ? PVP_ICONS.weapon.sword[obj.suit.weapon.sword][0] : "\ue17c"
    let axes_icon = (obj.suit.weapon.axe <= Object.keys(PVP_ICONS.weapon.axe).length-1) ? PVP_ICONS.weapon.axe[obj.suit.weapon.axe][0] : "\ue1c7"
    let misc_icon = (obj.suit.weapon.misc <= Object.keys(PVP_ICONS.weapon.misc).length-1) ? PVP_ICONS.weapon.misc[obj.suit.weapon.misc][0] : "\ue1c6"
    result = `${result}${sword_icon}` //Add sword icon
    result = `${result}${PVP_ICONS.weapon.bow[obj.suit.weapon.bow][0]}` //Add bow icon
    result = `${result}${axes_icon}` //Add axes icon
    result = `${result}${misc_icon}` //Add misc icon
    return result
}
...
export function pvpEditKitset(player,t,obj=pvpExportKit(t)) {
    ...
    form.slider(`\n${PVP_ICONS.weapon.arrow[0]} %item.arrow.name`,0,128,{valueStep:1, defaultValue:obj.suit.weapon.arrow})
    
    // Lefthand
    form.slider(`\n${PVP_ICONS.offhand.totem_of_undying[0]} %item.totem.name`,0,5,{valueStep:1, defaultValue:obj.suit.offhand.totem_of_undying})
    form.toggle(`${PVP_ICONS.offhand.shield[0]} %item.shield.name`,{defaultValue:Boolean(obj.suit.offhand.shield)})
    ...
    form.slider(`\n${PVP_ICONS.meal.golden_carrot[0]} %item.golden_carrot.name`,0,64,{valueStep:1, defaultValue:obj.suit.meal.golden_carrot})
    form.slider(`\n${PVP_ICONS.meal.golden_apple[0]} %item.golden_apple.name`,0,32,{valueStep:1, defaultValue:obj.suit.meal.golden_apple})
    form.slider(`\n${PVP_ICONS.meal.enchanted_golden_apple[0]} %item.appleEnchanted.name`,0,8,{valueStep:1, defaultValue:obj.suit.meal.enchanted_golden_apple})
...
export function pvpShowKitInfo(player,t=0,infotypeind=0,obj=pvpExportKit(t)) {
    ...
    form.button(`%item.${PVP_KITSUIT.weapon.axe[obj.suit.weapon.axe]}.name`,`${PVP_ICONS.weapon.axe[obj.suit.weapon.axe][1]}`)
    form.button(`%item.${PVP_KITSUIT.weapon.misc[obj.suit.weapon.misc]}.name`,`${PVP_ICONS.weapon.misc[obj.suit.weapon.misc][1]}`)
...
export async function pvpSettingGame(player) {
    try{
        ...
        .toggle('%axiscube.settings.pvp.regen',{defaultValue:Boolean(settingsRegen)})
        .toggle('%axiscube.settings.nametag',{defaultValue:Boolean(settingsNametag)})
        .toggle('%axiscube.settings.falldamage',{defaultValue:Boolean(settingsFalldamage)})
