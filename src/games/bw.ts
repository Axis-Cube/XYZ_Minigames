...
import {getPlayerTeam, teamArray, teamCheck, TEAMS4} from "./category_team"
import {
    Block,
    BlockPermutation,
    Dimension,
...
export function bwSettingGame(player,l=0) {
    if (l == 0) {
        ...
        for (let item in DB_DEFAULT.gen) {
            formEdit.slider(`${UNICODES[item]} %item.${item}.name (%gui.default: ${DB_DEFAULT.gen[item]/20})`,1,30,{valueStep:1, defaultValue:currentSettings.gen[item]/20})
        }
        ...
        for (let item in DB_DEFAULT.gen) {
            formEdit.slider(`${UNICODES[item]} %item.${item}.name (%gui.default: ${DB_DEFAULT.limits[item]})`,1,64,{valueStep:1, defaultValue:currentSettings.limits[item]})
        }
        formEdit.show(player).then( async gl => { if (!gl.canceled) {
            if(!gl.formValues){return;}
        ...
        for (let item in DB_DEFAULT.upgprice) {
            formEdit.slider(`${UNICODES[item]} %item.${item}.name (%gui.default: ${DB_DEFAULT.upgprice[item]})`,1,64,{valueStep:1, defaultValue:currentSettings.upgprice[item]})
        }
        formEdit.show(player).then( async gl => { if (!gl.canceled) {
            if(!gl.formValues){return;}
...
export function bwBlockBreak(player,block,brokenBlockPermutation) {
    ...
    } else if (BW_BLOCKS_T.indexOf(brokenBlockPermutation.type.id.split(':')[1]) == -1 && (player.getGameMode() != GameMode.Creative) ) {
...
export function bwBlockPlace(player,block,dimension) {
    ...
    if ((!BW_BLOCKS_T.includes(block.typeId.split(':')[1]) || BW_NOTPLACEABLE_BLOCKS.includes(downBlock.typeId.split(':')[1])) && (player.getGameMode() != GameMode.Creative) ) {
                            ...
                            switch(clickedItem.buy.type) {
                                ...
                                case 'slider':
                                    ...
                                    const bw_shop_sliderform = new ModalFormData()
                                        ...
                                        .slider(`${itemName.replaceAll('{{TEAM}}',getPlayerTeam(player))}`, clickedItem.buy.amount, item_max, {valueStep:clickedItem.buy.amount, defaultValue:clickedItem.buy.amount})
